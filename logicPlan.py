...
def atLeastOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    return conjoin(disjoin(literals))
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    res = []
    conjunctions = itertools.combinations(literals, 2)
    for i in conjunctions:
        res.append(disjoin(~i[0], ~i[1]))
    return conjoin(res)
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for coord in all_coords:
        ...
        # exactly one square
    possible_squares = [(PropSymbolExpr(pacman_str, coord[0], coord[1], time=t)) for coord in non_outer_wall_coords]
...
def positionLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #initial knowledge, location at  t=0
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))
    for t in range(50):
        ...
        KB.append(exactlyOne([PropSymbolExpr(pacman_str, coord[0], coord[1], time=t) for coord in non_wall_coords]))
        ...
        goalAssertion = findModel(goal & conjoin(KB))

        # If there is, return a sequence of actions from start to goal
        if goalAssertion:
            return extractActionSequence(goalAssertion, actions)
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        # true if and only if all of the food have been eaten
        goal = [~PropSymbolExpr(food_str, coord[0], coord[1], time=t) for coord in food]
        goalAssertion = findModel(conjoin(goal + KB))
        ...
        # Transition model sentences
        for coord in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(coord[0], coord[1], t + 1, walls))

        # food successor axiom
        for coord in food:
            pacman = PropSymbolExpr(pacman_str, coord[0], coord[1], time=t)
            currFood = PropSymbolExpr(food_str, coord[0], coord[1], time=t)
            nextFood = PropSymbolExpr(food_str, coord[0], coord[1], time=t+1)
            KB.append((currFood & pacman) >> ~nextFood)
            KB.append((currFood & ~pacman) >> nextFood)
